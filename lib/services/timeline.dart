  ...
  Widget build(BuildContext context) {
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        final dataIndicators = indicators;

        Widget? indicator;
        if (dataIndicators != null) {
          indicator = dataIndicators[index];
        }
...
class _TimelinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final bottom = size
        .topLeft(Offset(indicatorRadius, (indicatorSize - 10.0) + halfItemGap));
    final bottomBottom = size.bottomLeft(
      Offset(indicatorRadius, indicatorMargin - 5),
    );

    if (!isLast) canvas.drawLine(bottomBottom, bottom, linePaint);
